...
int main(void)
{
  ...
  long long int row_l, column_l, pont_atual, indice, contador = 0, status = 0, nao_sondou = 1;
  long row_r, column_r;
  int contador_dos_turnos = 0, cont_interno = 0;
  char command[10];
  long long int l, c;
  long long int chave;
  float pi = 3.14159265;
  ...
  //Pilha *pilha;
  FilaPrio *filaPrio;
  int TABLE_SIZE = 110000, EDAzinhos = 1, cont_turnos = 0;
  HTInit(110000);
  filaPrio = cria_FilaPrio();
  //pilha = cria_pilha();
  /* valores iniciais */
  ...
  */
  ...
  for (long t = 1; t < Turnos; t++)
  {
        ...
        for (c = (column_l + 1); c >= (column_l - 1); c--)
        {
          ...
          if (Item.tipo != 'D' && Item.tipo != 'S')
          {
            ...
            if (contador != t && status == 0)
            {
              ...
              Item.pontuacao = pont_atual;
              insere_FilaPrio(filaPrio, Item, pont_atual);
              indice = HTMudaChave2(chave);
              ht[indice].tipo = 'S';
              contador++;
            }
            else if (contador == t)
    ...
    else
    {
      ...
      //printf("resultado da consulta:%lld linha:%lld coluna%lld\n", consultar.pontuacao,consultar.pontuacao);
      ...
      for (l = (row_r + 1); l >= (row_r - 1); l--)
      {
        for (c = (column_r + 1); c >= (column_r - 1); c--)
        {
          //printf("row%d column%d ",row_r,column_r);
          //printf("Linha:%d Coluna:%d\n", l, c);
          chave = abs((((int)((l + 1) * pi) - (c + 1)) + ((int)(((c + 1) * pi) - (c + 1))) * (c + 1) + (l + 1)) % M);
          Item = HTsearch(chave);
          //printf("linha%d coluna%d tipo%c\n",Item.row,Item.column,Item.tipo);
          if (Item.tipo != 'D' && Item.tipo != 'S')
          {
            ...
            if (contador != t && status == 0)
            {
              ...
              HTinsert(Item, chave);
              fflush(stdout);
              scanf(" %s %ld %ld %ld", &command, &row_l, &column_l, &pont_atual);

              //printf("%ld %ld %ld \n",row_l, column_l,pont_atual);
              Item2.row = row_l;
              Item2.column = column_l;
              Item2.tipo = 'S';
              Item2.pontuacao = pont_atual;
              Item2.chave = chave;
              insere_FilaPrio(filaPrio, Item2, pont_atual);
              nao_sondou = 0;
              contador++;
            }
            else if (contador == t)
      ...
      if (nao_sondou == 1)
      {
        contador_dos_turnos = 1;
        while (nao_sondou == 1)
        {
          //printf("passou!!!");
          contador_dos_turnos++;
          if (contador_dos_turnos == t)
          {
            break;
          }
          if (contador_dos_turnos > 800)
          {
            contador_dos_turnos--;
            contador_dos_turnos = t - 1;
          }
          row_l = Dominados[contador_dos_turnos].row;
          column_l = Dominados[contador_dos_turnos].column;
          printf("veio da q");
          for (l = (row_l + 1); l >= (row_l - 1); l--)
          {
            for (c = (column_l + 1); c >= (column_l - 1); c--)
            {
              ...
              if (Item.tipo != 'D' && Item.tipo != 'S')
              {
                ...
                if (contador != t && status == 0)
                {
                  //printf("linha%lldcoluna%lld", Dominados[contador_dos_turnos].row, Dominados[contador_dos_turnos].column);
                  //printf("Linha:%d Coluna:%d\n", l, c);
                  // printf("aq%d", cont_interno);
                  printf("sondar %lld %lld\n", l, c);
                  Item.row = l;
                  Item.column = c;
                  Item.tipo = 'S';
