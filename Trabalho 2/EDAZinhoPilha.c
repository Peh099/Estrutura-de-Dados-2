...
int main(void)
{
  ...
  Matriz consultar;
  Matriz *Dominados;
  Dominados = (Matriz *)malloc(sizeof(Matriz) * 9000);
  //Pilha *pilha;
  FilaPrio *filaPrio;
  int EDAzinhos = 1, cont_turnos = 0;
  HTInit(38371);
  int L = 16657009;
  filaPrio = cria_FilaPrio();
  pilha = cria_pilha();
  ...
  chave = abs((((int)((Item.row + 1) * pi) - (Item.column + 1)) + ((int)(((Item.column + 1) * pi) - (Item.column + 1))) * (Item.column + 1) + (Item.row + 1)) % L);
  //printf("%ld",chave);
  Item.chave = chave;
  HTinsert(Item, chave);
  ...
  for (l = (row_l + 1); l >= (row_l - 1); l--)
  {
    ...
    {
      chave = abs((((int)((l + 1) * pi) - (c + 1)) + ((int)(((c + 1) * pi) - (c + 1))) * (c + 1) + (l + 1)) % L);
      Item = HTsearch(chave);
  ...
  */
  ...
  chave = abs((((int)((row_l + 1) * pi) - (column_l + 1)) + ((int)(((column_l + 1) * pi) - (column_l + 1))) * (column_l + 1) + (row_l + 1)) % L);
  ...
  for (long t = 1; t < Turnos; t++)
  {
    ...
    else
    {
      ...
      indice = HTMudaChave(chave);
      ht[indice].tipo = 'D';
      chave = abs((((int)((consultar.row + 1) * pi) - (consultar.column + 1)) + ((int)(((consultar.column + 1) * pi) - (consultar.column + 1))) * (consultar.column + 1) + (consultar.row + 1)) % L);
      Item = HTsearch(chave);
      //printf("tipo:%c\n",Item.tipo);
